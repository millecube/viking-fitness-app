...
export const AdminDashboard: React.FC<AdminDashboardProps> = ({ user }) => {
  ...
  const StatCard = ({ title, value, icon: Icon, trend }: any) => (
    <Card className="border-l-4 border-l-viking-action hover:scale-105 transition-transform duration-300">
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 border-b border-viking-grey/10 pb-6 animate-in slide-in-from-top duration-500">
        <div>
        ...
        <div className="flex items-center gap-3 bg-white/50 dark:bg-viking-blueLight/50 backdrop-blur-sm p-2 border border-viking-grey/20 rounded-xl shadow-sm">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 animate-in slide-in-from-bottom duration-500 delay-100">
        <StatCard 
          title="Total Members" 
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 animate-in slide-in-from-bottom duration-500 delay-200">
        <Card title={`Network Activity (${selectedBranchId === 'all' ? 'Global' : activeBranch?.name})`} className="h-80 shadow-xl">
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={activityData}>
              ...
              <Tooltip 
                contentStyle={{ backgroundColor: '#132436', borderColor: '#333', color: '#fff', borderRadius: '12px' }} 
                cursor={{ fill: '#333' }}
              />
              <Bar dataKey="active" fill="#0057B8" radius={[8, 8, 0, 0]} />
            </BarChart>
          </ResponsiveContainer>
        </Card>

        <Card title="Revenue Trend" className="h-80 shadow-xl">
          <ResponsiveContainer width="100%" height="100%">
            <LineChart data={revenueData}>
              <CartesianGrid strokeDasharray="3 3" stroke="#94A3B8" vertical={false} opacity={0.2} />
              <XAxis dataKey="name" stroke="#94A3B8" fontSize={12} tickLine={false} axisLine={false} tick={{fill: '#94A3B8'}} />
              <YAxis stroke="#94A3B8" fontSize={12} tickLine={false} axisLine={false} tick={{fill: '#94A3B8'}} />
              <Tooltip contentStyle={{ backgroundColor: '#132436', borderColor: '#333', color: '#fff', borderRadius: '12px' }} />
              <Line type="monotone" dataKey="val" stroke="#132436" strokeWidth={3} dot={{ fill: '#132436', strokeWidth: 2 }} className="dark:stroke-white" />
            </LineChart>
          </ResponsiveContainer>
        </Card>
      </div>
      ...
      <div className="animate-in slide-in-from-bottom duration-500 delay-300">
        <Card title="Branch List" className="shadow-xl">
              ...
              <tbody className="divide-y divide-viking-grey/10">
                {branches
                  ...
                  <tr key={branch.id} className="hover:bg-viking-offWhite dark:hover:bg-white/5 transition-colors">
                    ...
                    <td className="p-4">
                      <span className="inline-flex items-center gap-1.5 px-3 py-1 bg-green-100 dark:bg-green-900/30 text-green-600 dark:text-green-400 text-xs font-bold uppercase tracking-wider border border-green-200 dark:border-green-500/20 rounded-full">
                        Operational
                      </span>
                    </td>
