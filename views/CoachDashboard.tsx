...
export const CoachDashboard: React.FC<CoachDashboardProps> = ({ user }) => {
  ...
  return (
    <div className="space-y-6 animate-in slide-in-from-bottom-4 duration-500">
       <div className="flex justify-between items-center border-b border-viking-grey/10 pb-6">
        <div>
          ...
          <Card title={`My Roster (${students.length})`}>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {students.map(student => (
                <div key={student.id} className="bg-viking-offWhite dark:bg-viking-blueLight border border-viking-grey/20 p-4 flex items-center gap-4 hover:border-viking-action transition-all cursor-pointer group hover:bg-white dark:hover:bg-white/10 rounded-2xl">
          ...
          <Card title="Recent Student Activity">
            <div className="space-y-3">
              {workouts.slice(0, 5).map(workout => (
                 <div key={workout.id} className="flex items-center justify-between p-4 bg-viking-offWhite dark:bg-viking-blueLight border-l-4 border-l-red-500 rounded-lg">
            ...
            <div className="space-y-4 relative">
               ...
               {[9, 11, 14, 16].map((hour) => (
                 <div key={hour} className="flex items-start gap-4 relative z-10">
                    <span className="text-xs text-viking-grey w-12 text-right pt-2 font-bold font-mono">{hour}:00</span>
                    <div className="flex-1 p-3 bg-viking-offWhite dark:bg-viking-blueLight border-l-4 border-l-viking-action text-sm shadow-sm rounded-r-lg">
